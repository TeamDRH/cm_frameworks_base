...
class ProcessList {
    ...
    // Custom adjustment level 1
    static final int CUSTOM_LEVEL1_ADJ = 10;

    // Custom adjustment level 2
    static final int CUSTOM_LEVEL2_ADJ = 12;
    ...
    //};
    private final int[] mOomAdj = new int[] {
            FOREGROUND_APP_ADJ, VISIBLE_APP_ADJ, SERVICE_ADJ,
            CUSTOM_LEVEL1_ADJ, CUSTOM_LEVEL2_ADJ, HIDDEN_APP_MAX_ADJ
    };
    ...
    // Select use of custom OOM level limits.
    private final boolean useCustomOOM = true;
    
    // These are custom OOM level limits.  This is appropriate for a
    // custom build.  Values are in KB.
    private final long[] mOomMinFreeCustom = new long[] {
            8192, 14336, 56320,
            71680, 87040, 102400
    };
    ...
    private void updateOomLevels(int displayWidth, int displayHeight, boolean write) {
        ...
        for (int i=0; i<mOomAdj.length; i++) {
            ...
            if (useCustomOOM == true) {
                mOomMinFree[i] = mOomMinFreeCustom[i];
            } else {
