...
public class WifiTetherController extends MultipleStateController {
    ...
    public WifiTetherController(Context context, View button) {
        ...
        ((ImageView) button.findViewById(R.id.drh_settings_icon))
                    .setImageResource(R.drawable.drh_wifitether);
        updateController();
    }
    ...
    protected void setPreferenceStatus(final int status) {
        ...
        if (status == STATE_TURNING_ON
                && Settings.System.getInt(mContext.getContentResolver(),
                        "drh_quick_controls_airplane_wifi", 0) == 0
                && Settings.System.getInt(mContext.getContentResolver(),
    ...
    public void setSoftapEnabled(boolean enable) {
        ...
        if (enable && ((wifiState == WifiManager.WIFI_STATE_ENABLING) ||
            ...
            Settings.Secure.putInt(mResolver, Settings.Secure.WIFI_SAVED_STATE, 1);
        }
        /**
        ...
        if (!enable) {
            ...
            try {
                wifiSavedState = Settings.Secure.getInt(mResolver, Settings.Secure.WIFI_SAVED_STATE);
            } catch (Settings.SettingNotFoundException e) {
                ;
            }
            if (wifiSavedState == 1) {
                mWifiManager.setWifiEnabled(true);
                Settings.Secure.putInt(mResolver, Settings.Secure.WIFI_SAVED_STATE, 0);
            }
        }
    }
}
